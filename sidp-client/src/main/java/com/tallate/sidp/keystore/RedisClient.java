package com.tallate.sidp.keystore;

import java.util.Map;

/**
 * @author tallate
 */
public interface RedisClient {

    /**
     * 增/改
     */
    String set(final String key, final String value);

    /**
     * @param seconds 过期时间
     */
    String setex(final String key, final int seconds, final String value);

    /**
     * 设置过期时间，并且只有在不存在时才能保存成功
     */
    String setexnx(final String key, final int seconds, final String value);

    long hset(final String hkey, final String key, final String value);

    long incr(final String key);

    long decr(final String key);

    /**
     * 删
     */
    long del(final String key);

    long hdel(final String hkey, final String key);

    /**
     * 查
     */
    String get(final String key);

    String hget(final String hkey, final String key);

    Map<String, String> hgetAll(final String hkey);

    /**
     * 设置过期时间
     */
    long expire(final String key, final int second);

    /**
     * 获取有效时间,-1：永不过期，-2：已过期/已转移，n：还有n秒过期
     */
    long ttl(final String key);

    /**
     * 执行lua脚本
     * @param lua lua脚本
     * @return 执行结果
     */
    Object executeLua(final String lua);

}
